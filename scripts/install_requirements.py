...
if __name__ == '__main__':
    with open("../requirements.txt", "r") as file:
        ...
        packages = [f'{i.name}=={i.version}' for i in distributions()]
        not_installed = []
    ...
    if not_installed:
        ...
        try:
            ...
            for iteration in range(len(not_installed)):
                ...
                try:
                    subprocess.run(
                        [sys.executable, '-m', 'pip', 'install', not_installed[iteration]],
                    ...
                    print(f'Ошибка при установке {not_installed[iteration]}: {e.stderr}\n'
                          f'Прочитайте файл README.txt, чтобы узнать, как поступить в таком случае')
