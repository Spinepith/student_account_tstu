...
class ConsoleAccount(ConsoleAppSettings):
    ...
    def __login(self):
        ...
        if not self.__json_manager.find_file('user_data.json'):
            ...
            bad_symbols = '`~!@#$%^&*()-+={}[]:;"\'<>,.?/|\\'
            try:
                login = questionary.unsafe_prompt(input_login, true_color=True)['login']
                ...
                password = questionary.unsafe_prompt(input_password, true_color=True)['password']
    ...
    def __get_check_marks(self, all_lessons=False):
        ...
        if all_lessons:
            ...
            if not lessons:
                self.__warning_panel('Чтобы вернуться в меню, напишите "назад"')
